...
function populatesection(item){
  ...
  const article = `<article class ="card" onclick = "showModal(item)"> <h2>${item.title}</h2><figure> 
...
function showModal(item){
  const imageElement = document.querySelector("dialog:first-of-type > img:first-of-type")
  imageElement.src = item.image.large
}
